...
const ExerciseTwo = () => {
  ...
  interface CityDetail extends City {
    coords: Coords;
  }
  ...
  function getCityInfo(cityDetail: CityDetail) {
    const coords = `(${cityDetail.coords.latitude.toFixed(
      3
    )}, ${cityDetail.coords.longitude.toFixed(3)})`;
    return `${cityDetail.name.toUpperCase()} is located at ${coords}.`;
  }
